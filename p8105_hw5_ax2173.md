...
dataframe with estimated proportions and CIs for each city. \_\_\_\_
...
count_homicide_df = 
  ...
  mutate(
    prop = map(data, prop.test(n_unsolved_homicides, n_total, alternative = c("two.sided"), conf.level = 0.95)),
    results = map(prop, broom:tidy)
  )
```

nyc_airbnb %\>% nest(data = -borough) %\>% mutate( models = map(.x =
data, \~lm(price \~ stars + room_type, data = .x)), results =
map(models, broom::tidy) ) %\>% select(-data, -models) %\>%
unnest(results) %\>% filter(term != ‘(Intercept)’) %\>% select(borough,
term, estimate) %\>% pivot_wider( names_from = borough, values_from =
estimate )

prop_test = function(data) {

prop.test(n_unsolved_homicides, n_total, alternative = c(“two.sided”),
conf.level = 0.95)
